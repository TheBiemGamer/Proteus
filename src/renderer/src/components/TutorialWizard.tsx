...
export const TutorialWizard: React.FC<TutorialWizardProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 pointer-events-none flex flex-col justify-center items-center font-sans">
      ...
      <div className={`pointer-events-auto w-full max-w-2xl px-4 relative flex justify-center items-center transition-all duration-300 ${currentStep <= 1 ? 'min-h-[400px]' : 'h-0 min-h-0'}`}>
        {/* STEP 0: WELCOME */}
        <Transition
          as="div"
          show={currentStep === 0}
        ...
        >
          ...
          <p className="text-gray-300 text-lg">
            Your new modular mod manager. Let's get you set up with Nexus Mods integration for the best experience.
          </p>
        ...
        >
          <div className="flex flex-col items-center">
             <div className="bg-orange-500/10 p-4 rounded-full mb-4">
                 <img src={nexusIcon} className="w-12 h-12" alt="Nexus" style={{ filter: 'invert(0)' }}/>
             </div>
          ...
          <div className="text-left text-sm space-y-4">
            <div className="bg-gray-800 p-4 rounded-lg space-y-2">
                 ...
                 <a 
                  href="#" 
                  onClick={(e) => { e.preventDefault(); (window as any).electron.openUrl('https://www.nexusmods.com/settings/api-keys'); }} 
          ...
          <button onClick={() => setCurrentStep(2)} className="text-sm text-gray-500 hover:text-gray-300 underline">
        ...
        >
            <div className="relative h-40 w-full">
              ...
              <img
                src={previewMod?.imageUrl || (apiKey ? "https://staticdelivery.nexusmods.com/mods/1155/images/1108/1108-1669460592-1262846995.png" : "https://placehold.co/600x400/1e293b/475569?text=Mod+Cover")}
                alt="Mod Cover"
            ...
            <div className="p-6 flex-1 space-y-4 bg-[#0b1221]">
              <div className="flex flex-wrap gap-2">
                <span className="px-2 py-1 bg-white/5 rounded text-xs text-gray-400 border border-white/10">v{previewMod?.version || "5.2.0"}</span>
                <span className="px-2 py-1 bg-white/5 rounded text-xs text-blue-400 border border-white/10">{previewMod?.author || "Tobey"}</span>
                {apiKey ? (
                    <span className="px-2 py-1 bg-orange-900/40 text-orange-400 rounded text-xs border border-orange-700/40">NexusMods: 1108</span>
                ) : (
                    <span className="px-2 py-1 bg-yellow-900/40 text-yellow-400 rounded text-xs border border-yellow-700/40">Offline Mode</span>
                )}
              </div>
              <p className="text-gray-300 whitespace-pre-wrap leading-relaxed max-h-32 overflow-hidden text-sm">
                 {previewMod?.description || (apiKey ? "BepInEx is a plugin framework and mod loader for Unity Mono games. This pack is pre-configured for Subnautica." : "Mod description unavailable in offline mode.")}
              </p>
        ...
        >
          ...
          <p className="text-gray-300 mb-4">
            Here is your installed mod list. Notice how we have <span className="text-green-400">Nautilus</span> and <span className="text-green-400">Tobey's BepInEx</span> ready to go.
            {apiKey && <span className="block mt-2 text-blue-300 text-sm">Rich metadata makes managing mods easier!</span>}
          </p>
